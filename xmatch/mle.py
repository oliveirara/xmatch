...
def mle(
    ...
    # Ancillary match
    match_ancillary = xmatch(catalog_A, catalog_B, columns_A=columns_A, columns_B=columns_B, method="gc", radius=rs)
    # remove non-matching entries
    non_matches = match_ancillary.loc[:, ("AB", "separation")].isnull()
    match_ancillary = match_ancillary.loc[~non_matches]

    ancillary_sample = flatten_matches(match_ancillary, ("A", columns_A["id"]), ("B", columns_B["id"]))
    ...
    ancillary_sample = ancillary_sample.merge(catalog_A[[columns_A["id"], columns_A["pos_err"]]], on=columns_A["id"])

    # Background match
    match_background = xmatch(catalog_A, catalog_B, columns_A=columns_A, columns_B=columns_B, method="gc", radius=ro)
    # remove non-matching entries
    ...
    background_sample = flatten_matches(match_background, ("A", columns_A["id"]), ("B", columns_B["id"]))
    ...
    ancillary_sample[columns_B["id"]] = (
        ancillary_sample[columns_B["id"]].astype(float).astype(catalog_B[columns_B["id"]].dtype)
    )
    ancillary_sample = ancillary_sample.merge(catalog_B[[columns_B["id"], feature]], on=columns_B["id"])
    background_sample[columns_B["id"]] = (
        background_sample[columns_B["id"]].astype(float).astype(catalog_B[columns_B["id"]].dtype)
    )
    background_sample = background_sample.merge(catalog_B[[columns_B["id"], feature]], on=columns_B["id"])

    func_nm = surface_density(background_sample[feature], Area_total_background, False)

    func_qm = define_qm(
        ancillary_sample[feature], background_sample[feature], Area_total_ancillary, Area_total_background
    )

    func_LR = define_LR(func_qm=func_qm.evaluate, func_fr=func_fr, func_nm=func_nm.evaluate)

    # Alias
    df = ancillary_sample

    lr_col = f"LR_{feature}"
    ...
    if inner_join is True:
        ...
        B_matched = catalog_B.set_index(columns_B["id"]).loc[df_r.index.droplevel(0)].reset_index()
        B_matched.index = (
            catalog_A.set_index(columns_A["id"]).loc[df_r.index.droplevel(1)].reset_index()[columns_A["id"]]
        )
        AB_matched = df_r[matching_columns].reset_index(level=1, drop=True)

    df = concat([A_matched, B_matched, AB_matched], axis=1, keys=["A", "B", "AB_" + feature])
...
def flatten_matches(match_table, column_id_A, column_id_B):
    def expand_duplicates(group):
        ...
        """
        if len(group) != 1:
            raise ValueError("Group length is not 1")
        df_row = group.iloc[0]

        if ("AB", "duplicates") not in df_row.index:
            raise ValueError("Expected ('AB', 'duplicates') in df_row.index")
        if ("AB", "distances") not in df_row.index:
            raise ValueError("Expected ('AB', 'distances') in df_row.index")
        ...
        if df_row["AB", "duplicates"] is not None:
            ...
            dists_str = str(df_row["AB", "distances"]).split(";")
            seps.extend([float(sep) for sep in dists_str])
...
def build_features_catalog(sample_table, catalog, column_id):
    ...
    features_table = bgs.merge(catalog, left_index=True, right_on=column_id, sort=False, how="inner")
    features_table.set_index(column_id, inplace=True)
    return features_table


def define_qm(feature_data_ancillary, feature_data_background, surface_area_ancillary, surface_area_background):
    if feature_data_ancillary.ndim != 1:
        raise ValueError("feature_data_ancillary must be a 1-dimensional array")
    if feature_data_background.ndim != 1:
        raise ValueError("feature_data_background must be a 1-dimensional array")

    veca = feature_data_ancillary
    Area_total_ancillary = surface_area_ancillary
    vecb = feature_data_background
    Area_total_background = surface_area_background

    mininum = min(veca.min(), vecb.min())
    maximum = max(veca.max(), vecb.max())

    ha, ba = histogram(veca, bins=50, range=(mininum, maximum))
    ha = ha / Area_total_ancillary

    hb, _ = histogram(vecb, bins=50, range=(mininum, maximum))
...
def define_qms(acs_magnitudes, bkg_magnitudes, columns, Area_total_ancillary, Area_total_background):
    """
    Return compiled versions of q(m) for each of 'columns'
    """
    func_qm = {}
    for col in columns:  # ['YAPERMAG3','J_1APERMAG3','HAPERMAG3','KAPERMAG3']:
        func_qm[col] = define_qm(acs_magnitudes[col], bkg_magnitudes[col], Area_total_ancillary, Area_total_background)
    return func_qm
